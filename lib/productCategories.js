...
/** Map legacy DB category values to current shop category names */
export function normalizeProductCategory(category) {
  if (!category) return "3D Printed Wordart";
  if (isFilamentCategory(category)) return FILAMENT_CATEGORY;
  if (category === "Accessories") return "3D Printed Gaming Accessories";
  return category;
}
